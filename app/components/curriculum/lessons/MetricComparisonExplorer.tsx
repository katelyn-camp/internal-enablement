...
function ComparabilityBadge({ comparability }: { comparability: MetricProviderEntry["comparability"] }) {
  ...
  return (
    <span
      className={`mt-1 inline-flex items-center rounded-full px-2 py-0.5 text-[0.6rem] font-medium tracking-wide normal-case ${
        isSame ? "bg-forest text-paper" : "border border-dashed border-line text-ink/45"
      }`}
    >
...
function AirOpsDefinition({ airOps }: { airOps: MetricProviderEntry }) {
  return (
    <div className="mb-4 rounded-card border border-line bg-paper-2 p-4">
      <div className="mb-1 text-caption font-medium tracking-wide text-ink/45 uppercase">AirOps&rsquo; definition</div>
      <p className="text-sm leading-relaxed text-ink/80">
        <span className="font-medium text-ink">{airOps.termUsed}: </span>
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-card border border-line">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              <th className={`${LABEL_COL_CLASSES} bg-paper-2 py-2.5 text-left text-caption font-medium tracking-wide text-ink/50 uppercase`} />
              {category.providers.map((p) => (
                <th
                  key={p.provider}
                  className={`min-w-[220px] px-3 py-2.5 text-left align-top text-caption font-medium tracking-wide uppercase ${
                    p.provider === "AirOps" ? "bg-forest text-paper" : "bg-paper-2 text-ink/50"
                  }`}
                >
          ...
          <tbody className="divide-y divide-line">
            {ROWS.map((row) => (
              <tr key={row.key}>
                <td className={`${LABEL_COL_CLASSES} bg-paper font-medium text-ink`}>{row.label}</td>
                {category.providers.map((p) => (
                  <td
                    key={p.provider}
          ...
          <tfoot>
            <tr className="border-t border-line">
              <td className={`${LABEL_COL_CLASSES} bg-paper-2 py-2.5 text-[0.7rem] font-medium tracking-wide text-ink/45 uppercase`}>Source</td>
              {category.providers.map((p) => (
      ...
      <div className="mb-4 flex flex-wrap justify-center gap-2">
        {METRIC_CATEGORIES.map((c) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-xs font-medium tracking-wide uppercase transition-colors ${
              c.id === activeId ? "border-forest bg-forest text-paper" : "border-line bg-paper-2 text-ink/60 hover:border-ink/25"
            }`}
          >
            {c.label}
          </button>
