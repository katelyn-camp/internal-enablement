...
export const M7_MANAGED_SERVICES_KNOWLEDGE_CHECK: KnowledgeCheckQuestion[] = [
  ...
  },
  {
    ...
    modelAnswer:
      "A buyer influenced by an AI-search answer often doesn't convert on that exact touch, especially on upper- or mid-funnel queries, they read the answer, form an opinion, and frequently continue the journey through another channel, coming back later through a branded search or a direct visit, though a high-intent query can convert in-session too. When it doesn't, the AI-search touch is unlikely to ever be the last touch before conversion, so last-touch assigns it $0 by construction, regardless of how much it actually shaped the decision. This is why a client relying only on last-click GA numbers can look like AI search is doing nothing, even when it's the reason the buyer showed up in the first place, and why it matters to know which model a client's account is using before treating that number as the whole story.",
  },
];
